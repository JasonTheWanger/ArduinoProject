...
void setup(void)
{
  ...
  if (myFile) {
    ...
    myFile.println("Time,CO2,TVOC,Temperature,Humidity,Pressure,Gas_Res,PM2.5");
  }
  myFile.close();
...
void loop() {
  ...
  if (myFile) {
    if (timer >= threshold) {
      ...
      myFile.print(",");
      myFile.print(sumGas / 100.000 / threshold);
      myFile.print(",");
      myFile.println(luxSum / threshold);
      myFile.print(",");
      myFile.println(PMsum / threshold);
      Serial.println("done.");
      timer = 0;
      sumCO2 = 0.000;
      sumTVOC = 0.000;
